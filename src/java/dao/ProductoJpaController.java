...
public class ProductoJpaController implements Serializable {
    ...
    public void create(Producto producto) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            em.persist(producto);
            if (categoria != null) {
                categoria.getProductoCollection().add(producto);
                categoria = em.merge(categoria);
            }
            for (Detalle detalleCollectionDetalle : producto.getDetalleCollection()) {
                Producto oldIdProductoOfDetalleCollectionDetalle = detalleCollectionDetalle.getIdProducto();
                detalleCollectionDetalle.setIdProducto(producto);
                detalleCollectionDetalle = em.merge(detalleCollectionDetalle);
                if (oldIdProductoOfDetalleCollectionDetalle != null) {
                    oldIdProductoOfDetalleCollectionDetalle.getDetalleCollection().remove(detalleCollectionDetalle);
                    oldIdProductoOfDetalleCollectionDetalle = em.merge(oldIdProductoOfDetalleCollectionDetalle);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Detalle detalleCollectionOldDetalle : detalleCollectionOld) {
                if (!detalleCollectionNew.contains(detalleCollectionOldDetalle)) {
                    ...
                    illegalOrphanMessages.add("You must retain Detalle " + detalleCollectionOldDetalle + " since its idProducto field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            if (categoriaOld != null && !categoriaOld.equals(categoriaNew)) {
                categoriaOld.getProductoCollection().remove(producto);
                categoriaOld = em.merge(categoriaOld);
            }
            if (categoriaNew != null && !categoriaNew.equals(categoriaOld)) {
                categoriaNew.getProductoCollection().add(producto);
            ...
            for (Detalle detalleCollectionNewDetalle : detalleCollectionNew) {
                if (!detalleCollectionOld.contains(detalleCollectionNewDetalle)) {
                    Producto oldIdProductoOfDetalleCollectionNewDetalle = detalleCollectionNewDetalle.getIdProducto();
                    detalleCollectionNewDetalle.setIdProducto(producto);
                    detalleCollectionNewDetalle = em.merge(detalleCollectionNewDetalle);
                    if (oldIdProductoOfDetalleCollectionNewDetalle != null && !oldIdProductoOfDetalleCollectionNewDetalle.equals(producto)) {
                        oldIdProductoOfDetalleCollectionNewDetalle.getDetalleCollection().remove(detalleCollectionNewDetalle);
                        oldIdProductoOfDetalleCollectionNewDetalle = em.merge(oldIdProductoOfDetalleCollectionNewDetalle);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Detalle detalleCollectionOrphanCheckDetalle : detalleCollectionOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Producto (" + producto + ") cannot be destroyed since the Detalle " + detalleCollectionOrphanCheckDetalle + " in its detalleCollection field has a non-nullable idProducto field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
            Categoria categoria = producto.getCategoria();
            if (categoria != null) {
                categoria.getProductoCollection().remove(producto);
                categoria = em.merge(categoria);
            }
            em.remove(producto);
            utx.commit();
